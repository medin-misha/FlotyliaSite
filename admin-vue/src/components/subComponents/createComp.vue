...
<template>
  <main class="flex min-h-screen w-full items-start justify-center bg-background px-margin-desktop pb-12 pt-[100px] dark:bg-on-background">
    <div class="w-full max-w-4xl bg-surface-container-lowest dark:bg-[#16181c] rounded-xl border border-outline-variant/30 dark:border-white/10 shadow-[0_12px_32px_rgba(175,16,26,0.04)] overflow-hidden flex flex-col transition-all duration-300">

      <!-- Card Header -->
      <div class="p-stack-lg border-b border-outline-variant/30 dark:border-white/10 flex justify-between items-center bg-surface-container-lowest dark:bg-[#16181c]">
        <div>
          <h2 class="font-headline-md text-headline-md font-bold text-on-surface dark:text-white flex items-center gap-2 select-none">
            <span class="material-symbols-outlined text-[28px] text-primary dark:text-inverse-primary">person_add</span>
            <span>Создать запись</span>
          </h2>
          <p class="font-body-md text-body-md text-secondary dark:text-secondary-fixed-dim mt-0.5 select-none">
            Раздел: {{ pageStore.pageData.name === 'Users' ? 'Пользователи' : 'Администраторы' }}
          </p>
        </div>
        <div class="px-4 py-1.5 rounded-full bg-primary/10 text-primary dark:text-inverse-primary font-label-md text-label-md font-bold uppercase tracking-wider select-none animate-pulse">
          Новая запись
        </div>
      </div>
      ...
      >
        <div v-if="globalError" class="mx-6 mt-6 p-4 rounded-xl border border-error/20 bg-error/5 text-error flex items-start gap-3 shadow-sm">
          ...
          <button @click="globalError = null" class="text-error hover:opacity-85 shrink-0 transition-opacity">
      ...
      <div v-if="isUsers" class="px-6 pt-5 border-b border-outline-variant/20 dark:border-white/5 bg-surface-container-lowest dark:bg-[#16181c]">
        <nav class="flex flex-wrap gap-2 select-none" aria-label="Tabs">
          ...
          >
            ...
            <span
              ...
              :class="currentTab === tab.id ? 'ring-white dark:ring-primary' : 'ring-background dark:ring-inverse-surface'"
            ></span>
          </button>
        </nav>
      </div>
      ...
      <div class="flex-1 p-stack-lg max-h-[calc(100vh-300px)] overflow-y-auto custom-scrollbar bg-surface-container-lowest dark:bg-[#16181c]">
      ...
      <!-- Sticky Footer -->
      <div class="p-stack-lg border-t border-outline-variant/30 dark:border-white/10 bg-surface-container-lowest dark:bg-[#16181c] flex items-center justify-end gap-stack-md sticky bottom-0 z-10 shadow-[0_-4px_16px_rgba(0,0,0,0.02)]">
        <button
