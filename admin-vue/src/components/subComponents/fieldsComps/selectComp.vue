...
<template>
  ...
  <div v-if="create" class="flex flex-col w-full gap-1.5 group">
    ...
    <div class="relative flex items-center">
      ...
      >
        <option disabled value="" class="bg-white dark:bg-[#1f2125] text-on-surface dark:text-white">
      ...
      <span class="material-symbols-outlined absolute right-3 text-secondary/50 pointer-events-none">
        keyboard_arrow_down
      </span>
    </div>
  ...
  <div v-else :class="['p-4 rounded-xl border bg-surface-container-low dark:bg-white/5 flex flex-col gap-1 relative group hover:scale-[1.01] hover:shadow-sm transition-all duration-200', error ? 'border-error ring-1 ring-error/50 bg-error/5 dark:bg-error/10' : 'border-outline-variant/30']">
    ...
    <div class="flex items-center justify-between gap-2 mt-0.5">
      ...
      >
        <option v-for="option in field.options" :key="option.value" :value="option.value" class="bg-surface-container-lowest dark:bg-inverse-surface text-on-surface dark:text-white">
          {{ option.label }}
        </option>
      </select>
