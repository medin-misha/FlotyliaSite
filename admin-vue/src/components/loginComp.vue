...
<template>
  <section
    class="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-[#111315] p-margin-mobile"
  >
    <div class="absolute inset-x-0 top-0 h-1 bg-primary"></div>

    <div
      class="z-10 flex w-full max-w-[420px] flex-col items-center rounded-xl border border-white/10 bg-[#181b1f] p-stack-lg shadow-2xl transition-all"
    >
      <!-- Brand Icon & Titles -->
      <div
        class="w-14 h-14 rounded-2xl bg-primary flex items-center justify-center text-white mb-6 shadow-lg shadow-primary/30"
      >
        <span
      ...
      <p class="text-xs text-secondary/60 mb-8 uppercase tracking-widest font-semibold">
        Flotylia Suite
      </p>
      ...
      <form @submit.prevent="loginUser" class="w-full space-y-5">
        ...
        <div class="space-y-1">
          ...
          <div class="relative">
            <span
              class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-secondary/50 text-[18px]"
              >person</span
            >
            <input
              v-model="login"
        ...
        >
          ...
          <span v-if="!isSubmitting" class="material-symbols-outlined text-[16px]"
            >arrow_forward</span
          >
        </button>
      </form>
